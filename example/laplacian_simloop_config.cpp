...
int main(int argc, char** argv) {
	...
	ros::init(argc, argv, "laplacian_simloop_config");

	std::string datapath;
	int framesize;
	
	rosneuro::Filter<double>* laplacian = new rosneuro::Laplacian<double>();
	if(laplacian->configure("Laplacian") == false) {
	...
	const std::string fileout   = datapath + "/test/laplacian_simloop_config.csv";
